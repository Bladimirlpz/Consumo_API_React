...
function Buscador() {
  ...
  const filtroFarmacia = data.filter((farmacias) => farmacias.local_nombre.toLowerCase() === farmacia.toLowerCase());

  // INPUT Y FILTRADO POR COMUNA
  const inputComuna = (e) => { 
    setComuna(e.target.value)
  };
  const filtroComuna = data.filter((farmacias) => farmacias.comuna_nombre.toLowerCase() === comuna.toLowerCase());

  return (
    <>
      <section className="seccion-buscador">
        <h1>Buscador</h1>
        <InputGroup size="sm" className="mb-2 input-farmcia" value={farmacia} onChange={inputFarmacia}>
          <InputGroup.Text style={{ width: '5rem' }} id="inputGroup-sizing-sm">Farmacia</InputGroup.Text>
          <Form.Control
        ...
        <InputGroup size="sm" className="mb-2 input-comuna" value={comuna} onChange={inputComuna}>
          <InputGroup.Text style={{ width: '5rem' }} id="inputGroup-sizing-sm">Comuna</InputGroup.Text>
          <Form.Control
